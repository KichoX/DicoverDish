...
function Icon({ name, size = 18 }: { name: string; size?: number }) {
  const paths: Record<string, React.ReactNode> = {
    ...
    shield:   <path d="M12 2 3 7v6c0 5 3.5 9.7 9 11 5.5-1.3 9-6 9-11V7Z" />,
...
function LogoMark() {
  return (
    <svg width="30" height="22" viewBox="0 0 2733 2035" fill="none" aria-hidden="true">
      ...
      <path fillRule="evenodd" clipRule="evenodd"
        ...
        fill="currentColor" />
      <path d="M2679.76 1860.64C2709.16 1860.64 2733 1899.54 2733 1947.52C2733 1995.5 2709.16 2034.39 2679.76 2034.39L53.2395 2034.39C23.8357 2034.39 0 1995.5 0 1947.52C0 1899.54 23.8357 1860.64 53.2395 1860.64L2679.76 1860.64Z"
        fill="currentColor" />
    </svg>
  )
}
...
export function SiteHeader() {
  ...
  // Client-only nav links (hide for admin/superadmin/guest)
  const showClientNav = isLoggedIn && user?.role === 'client'
  ...
  return (
    ...
    >
      <div className="max-w-[1280px] mx-auto px-8 flex items-center gap-6" style={{ height: 68 }}>
...
<<<<<<< Updated upstream
          ...
          >
=======
          <span className="hidden sm:inline text-[10px] font-medium uppercase tracking-[0.2em] self-center pl-2"
            style={{ borderLeft: '1px solid var(--hairline)', color: 'var(--muted-ink)', fontFamily: 'var(--font-sans)' }}>
>>>>>>> Stashed changes
            Berlin Ed.
          </span>
        </Link>
        ...
        <Link href="/search"
          ...
          style={{
            ...
            borderRadius: 999, color: 'var(--muted-ink)', fontSize: 14, textDecoration: 'none',
          }} aria-label="Search">
        ...
        <div className="flex items-center gap-[22px] ml-auto">
          ...
          {/* Client nav links */}
          {showClientNav && navLinks.map((link) => {
            const active = pathname === link.href
            return (
              <Link key={link.href} href={link.href}
                className="hidden md:block text-sm relative"
                style={{ color: active ? 'var(--ink)' : 'var(--ink-2)', fontWeight: active ? 500 : 400, padding: '6px 0', textDecoration: 'none' }}>
                {link.label}
              </Link>
            )
          })}

          {/* Cart — only for guests and clients */}
          {(!isLoggedIn || user?.role === 'client') && (
            <Link href="/search" className="relative" style={{ color: 'var(--ink-2)', display: 'inline-flex' }} aria-label="Cart">
              <Icon name="bag" size={20} />
              {cartCount > 0 && (
                <span className="absolute" style={{
                  top: -6, right: -10, background: 'var(--brand)', color: '#fff',
                  borderRadius: 999, fontSize: 10, minWidth: 16, height: 16,
                  padding: '0 4px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600,
                }}>
          ...
          {isLoggedIn ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-1.5 cursor-pointer" style={{
                  height: 34, padding: '0 14px',
                  background: 'var(--paper)', border: '1px solid var(--hairline)',
                  borderRadius: 999, fontSize: 13, color: 'var(--ink)', gap: 6,
                }}>
              ...
              <DropdownMenuContent align="end" className="w-52">
                <DropdownMenuLabel>
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs font-medium capitalize" style={{ color: 'var(--brand)' }}>
                ...
                {user?.role === 'superadmin' && (
                  <>
                    <DropdownMenuItem onClick={() => router.push('/super-admin')}>
                      <span className="mr-2"><Icon name="shield" size={14} /></span>Super Admin
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                  </>
                )}
